...
# Checking if system is 32bit or 64bit
sysCheck=$(lscpu | grep "Architecture" | awk -F: {'print $2'} | sed 's/ //g')
if [ "$sysCheck" != "x86_64" ]
then
	echo "Error: System is 32bit not compatible with nodejsMe"
else
	:
fi
...
function userHelp(){
	...
	printf "\t uninstall\t Uninstalls nodejs from system.\n"
...
function nodejsInstall(){	
	...
	cd $myDir && rm -r node-$(cat .njsme)-linux-x64 && rm node-$(cat .njsme)-linux-x64.tar.xz
}
